...
string getRandomWord() {
    ...
    string words[] = {"dictionary", "accept", "reject", "curriculum", "subway", "program", "database", "incubator"};
    // Generate a random index between 0 and 7 to pick a word from the array
    int index = rand() % 8;
    return words[index];  // Return the randomly chosen word
}
int main() {
    srand(time(0));  // Initialize random number generator using the current
    // time (seeds the random number generator)
   ...
   // generating a welcome message for the user
    ...
    for (int i = 0; i < totalWords; i++) {
        ...
        cout << "Type this word: ";  // ask the player to type the word
        ...
        if (userInput == wordToType) {
            ...
            // Display the correct word if the player typed wrong
